...
public abstract class RTPCodec {
    ...
    /////
    private int find_best_length_mipi(byte data[], int offset, int length) {
        //see if there is a 0,0,1 and return a length to that
        //this way the next packet will start at a resync point
        for (int a = 1; a < length - 3; a++) {
            if (data[offset + a] == 0 &&
     ...
     */
    public byte[][] encode(byte data[], int payloadType, int frameRate) {
        ...
        if (frameRate != 0) timestamp += 90000 / frameRate;  //??? 10 fps ???
    ...
    protected static int gettimestamp(byte[] data, int off) {
    ...
    public byte[] decode(byte rtp[], int length) {
        ...
        byte type = (byte) (rtp[fu_header_len] & 0x1f);
        ...
        if (type == 28) {
            ...
            ret = new byte[h264Length + 4];
            ret[0] = 0;
            ret[1] = 0;
            ret[2] = 0;
            ret[3] = 1;
            System.arraycopy(rtp, fu_header_len, ret, 4, h264Length);
        }

        return ret;
    }

    public abstract byte[] rtpToNalu(byte rtp[], int length);
    ...
    public byte frameType(byte[] frame) {
        return (byte) ((frame != null && frame.length > 4) ? (frame[4] & 0x1F) : 0xFF);
    }
