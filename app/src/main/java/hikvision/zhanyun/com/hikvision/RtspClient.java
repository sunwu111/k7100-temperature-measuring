...
public class RtspClient {
    ...
    public static class SDPInfo {
        public int videoPayloadType = -1;
        public int audioPayloadType = -1;
    ...
    private static String extractValue(String line, String key) {
        ...
        String v = line.substring(idx + key.length() + 1);
        int end = v.indexOf(";");
        return end > 0 ? v.substring(0, end) : v;
    }
...
//                            Log.e(TAG, "RTP 回调错误: " + e.getMessage());
                        ...
                        ///////////////////////////// sunwu  解析SDP信息  获取音视频相关的参数信息
//                        Log.i(TAG, "收到 RTSP 响应: " + sb.toString() + "\r\n" + new String(body));

                        String header = sb.toString();
                        String bodyStr = body != null ? new String(body) : "";
                        boolean isSdp = header.contains("Content-Type: application/sdp") || bodyStr.startsWith("v=0");
                        ...
                        if (isSdp) {
                            ...
                            for (String sdpLine : lines) {
                                sdpLine = sdpLine.trim();

                                if (sdpLine.startsWith("m=video")) {
                                    isVideo = true;
                                    isAudio = false;
                                    sdpInfo.videoPayloadType = Integer.parseInt(sdpLine.split(" ")[3]);
                                } else if (sdpLine.startsWith("m=audio")) {
                                    isVideo = false;
                                    isAudio = true;
                                    sdpInfo.audioPayloadType = Integer.parseInt(sdpLine.split(" ")[3]);
                                } else if (sdpLine.startsWith("a=control:")) {
                                    String ctrl = sdpLine.substring(10);
                                    if (isVideo) sdpInfo.videoControl = ctrl;
                                    if (isAudio) sdpInfo.audioControl = ctrl;
                                } else if (sdpLine.startsWith("a=rtpmap:")) {
                                    String[] p = sdpLine.substring(9).split(" ");
                                    String codec = p[1];

                                    if (isAudio && codec.startsWith("mpeg4-generic")) {
                                        String[] a = codec.split("/");
                                        sdpInfo.audioSampleRate = Integer.parseInt(a[1]);
                                        sdpInfo.audioChannels = Integer.parseInt(a[2]);
                                    }
                                } else if (sdpLine.startsWith("a=fmtp:")) {
                                    if (isVideo && sdpLine.contains("sprop-parameter-sets")) {
                                        String v = sdpLine.substring(
                                                sdpLine.indexOf("sprop-parameter-sets=") + "sprop-parameter-sets=".length());
                                        String[] sp = v.split(",");
                                        sdpInfo.sps = Base64.decode(sp[0], Base64.DEFAULT);
                                        sdpInfo.pps = Base64.decode(sp[1], Base64.DEFAULT);
                                    }

                                    if (isAudio && sdpLine.contains("config=")) {
                                        String cfg = extractValue(sdpLine, "config");
                                        sdpInfo.audioSpecificConfig = hexToBytes(cfg);
                                    }
                                } else if (isVideo && sdpLine.startsWith("a=x-dimensions:")) {
                                    String[] wh = sdpLine.substring(15).split(",");
                                    sdpInfo.width = Integer.parseInt(wh[0]);
                                    sdpInfo.height = Integer.parseInt(wh[1]);
                                }
                            }
                        }
                        ///////////////////////////
    ...
    private static byte[] audioConfigToBytes(String cfg) {
        // cfg 是十进制字符串，如 "1408"
    ...
    public boolean send(String request, String cmd) {
        try {
            ...
            Log.e(TAG, "发送 RTSP 请求 " + cmd + " 错误: " + e.getMessage());
            return false;
        }
    }
     ...
     */
    private boolean setup(boolean useAudio) {
        try {
            String requestVideo;
            String requestAudio = null;
            if (udpMode) {
                rtpSocket = new DatagramSocket();
                requestVideo = String.format("SETUP %s RTSP/1.0\r\nTransport: RTP/AVP;unicast;client_port=%d-%d\r\n",
                ...
                if (useAudio) {
                    requestAudio = String.format("SETUP %s RTSP/1.0\r\nTransport: RTP/AVP/TCP;unicast;interleaved=2-3\r\n", audioControl);
                }
            }
            if (!send(requestVideo, "SETUP")) return false;
            if (useAudio) {
                if (!send(requestAudio, "SETUP")) return false;
            }
...
//        if (!setup()) return false; /////
     ...
     */
    private boolean describe(boolean useAudio) {
        ...
        try {
            ...
            for (String s : ss) {
                if (s.startsWith("m=video")) {
                    isVideo = true;
                    isAudio = false;
                } else if (s.startsWith("m=audio")) {
                    isVideo = false;
                    isAudio = true;
                } else if (s.startsWith("a=control:")) {
                    String ctrl = s.substring("a=control:".length());
                    if (isVideo) videoControl = ctrl;
                    if (isAudio) audioControl = ctrl;
                    control = s.replace("a=control:", "");
                }
//                if (s.toLowerCase().contains("h264") || s.toLowerCase().contains("h265"))
//                    break;
            }
            if (!videoControl.toLowerCase().startsWith("rtsp://")) videoControl = uri + "/" + videoControl;
            if (useAudio) {
                if (!audioControl.toLowerCase().startsWith("rtsp://")) audioControl = uri + "/" + audioControl;
            }
